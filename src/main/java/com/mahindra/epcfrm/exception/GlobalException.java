...
public class GlobalException {
	...
	public ResponseEntity<?> handleDataNotFoundException(DataNotFoundException exception, WebRequest request) {
		ErrorDetails details = new ErrorDetails(new Date(), exception.getMessage(), request.getDescription(false), null,
				404, false);
	...
	public ResponseEntity<?> handleDataNotFoundException(IOException exception, WebRequest request) {
		ErrorDetails details = new ErrorDetails(new Date(), exception.getMessage(), request.getDescription(false), null,
				404, false);
		return new ResponseEntity<>(details, HttpStatus.NOT_FOUND);
	}
	...
	public ResponseEntity<ErrorDetails> handleRunTimeException(RuntimeException exception, WebRequest request) {
		ErrorDetails details = new ErrorDetails(new Date(), exception.getMessage(), request.getDescription(false), null,
				404, false);
		return new ResponseEntity<>(details, HttpStatus.INTERNAL_SERVER_ERROR);
	}
